...
class Graph extends Component {
  ...
  resize = (size = getSize()) => {
    ...
    if (graphData.svgCanvas)
      graphData.svgCanvas.attr("width", width).attr("height", height);
  ...
  render() {
    if (this.props.nodesChanged) {
      ...
      let { minRadius, maxRadius } = updateNodes(
        this.props.nodes,
        this.props.options
      );
  ...
  _createTooltip() {
    ...
    graphData.tooltip = graphData.svgCanvas.append("g").style("opacity", 0);
  ...
  _createCanvas(width, height) {
    graphData.svgCanvas = d3
      .select(graphData.rootNode)
      .append("svg")
      .attr("width", width)
      .attr("height", height)
  ...
  _createSimulation(width, height) {
    ...
    graphData.simulation = d3
      .forceSimulation(graphData.props.nodes)
      .force(
        "link",
        d3
          .forceLink(graphData.props.links)
          .id((d) => d.id)
          .distance(
            (link) =>
              scale(link.source.radius) +
              scale(link.target.radius) +
              minDimension / SIMULATION.DISTANCE_MAX_RATIO
          )
      ...
      .force(
        ...
        d3
          ...
          .strength(
            (d) => -1 * SIMULATION.STRENGTH * Math.sqrt(scale(d.radius))
          )
      )
  ...
  _setGraphData() {
    ...
    graphData.link = graphData.svgCanvas
      .append("g")
      .attr("stroke", "#999")
      .selectAll("line")
    ...
    graphData.node = graphData.svgCanvas
      .append("g")
      .selectAll("g")
    ...
    graphData.arrow = graphData.svgCanvas
      .append("g")
      .selectAll("line")
      .data(graphData.props.links)
      .join("line")
  ...
  _startGraph() {
    graphData.simulation.on("tick", () => {
      ...
      graphData.node.attr("transform", (d) => `translate(${d.x},${d.y})`);
